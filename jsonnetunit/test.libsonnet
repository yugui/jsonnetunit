...
local suite(tests) = {
    ...
    verify: (
        local failures = [
            tc for tc in self.result if !tc.matcher.matches(tc.expectationType)
        ];
        if std.length(failures) > 0 then
            local message = "Failed %d/%d test cases:\n" % [
                std.length(failures), std.length(self.result),
            ] + std.join("\n", [
                "%s: %s" % [tc.name, tc.matcher.message(tc.expectationType)] for
                tc in failures
            ]);
            error message
        else
